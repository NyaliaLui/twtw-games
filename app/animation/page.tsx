...
export default function Animation() {
  ...
  return (
    <div className="fixed inset-0 bg-black z-40">
      <Canvas shadows={animationConfig.enableShadows} camera={{ position: animationConfig.camera.position, fov: animationConfig.camera.fov }}>
        <ambientLight intensity={animationConfig.ambientLight.intensity} />
        <directionalLight castShadow position={animationConfig.directionalLight.position} intensity={animationConfig.directionalLight.intensity} />

        <Soldier keys={keysRef.current} gltfPath='/models/Soldier.glb' />
        <World />

        <OrbitControls enableRotate={animationConfig.orbitControls.enableRotate} target={animationConfig.orbitControls.target} />
      </Canvas>
